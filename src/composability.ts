...
export async function loadComposabilityClaims(input: {
  ...
  try {
    ...
    const directClaims = (payload.data?.triples ?? []).map((triple) => ({
      ...
      opposition: signal(triple.counter_term?.vaults),
      createdAt: triple.created_at ?? "",
      context: [] as ComposabilityContextClaim[],
    }));

    const contextIds = Object.values(input.contextPredicateIds ?? {}).filter(
      (value): value is string => Boolean(value?.trim()),
    );
    if (contextIds.length && directClaims.length) {
      const contextResponse = await fetcher(input.endpoint, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
      ...
      if (!contextResponse.ok) {
        return {
          kind: "error",
          message: `Registry context request failed (${contextResponse.status}).`,
        };
      }
      ...
      const claimsById = new Map(
        directClaims.map((claim) => [claim.id, claim]),
      );
      for (const triple of contextPayload.data?.triples ?? []) {
        const parent = claimsById.get(triple.subject_id ?? "");
        if (parent)
          parent.context.push(
            contextClaimFromTriple(triple, input.contextPredicateIds ?? {}),
          );
      }
    }
